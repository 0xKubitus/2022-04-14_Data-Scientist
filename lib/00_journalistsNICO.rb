...
puts "le handle le plus court de cette liste est : #{array[0]}"

puts "ce handle fait : #{array[0].length} caractères."
print "\n"
...
new_array = array.each do |handle|
    if handle.slice!(0) != "@"
      puts "ATTENTION MEC Y'A UN PIEGE !"
    end
  ...
  puts "il y a #{reponse3} handle de 5 caractères."
print "\n"

# 4) 
puts "Question 4 = Combien commencent par une majuscule (première lettre juste après le @) ?"
reponse4 = 0
new_array.each do |handle|
    if handle[1] =~ /[A-Z]/
      reponse4 += 1
    end
end
  puts "il y a #{reponse4} de handle qui commencents par une majuscule."
print "\n"
...
puts reponse5.sort
puts "il y a toujours #{reponse5.length} handle dans l'Array ;) "
...
# équivalent à: puts array.sort_by {|x| x.length}
...
puts ' ' 
puts "Je t'invite a scroll un toooooooooout petit peu pour accéder aux premières réponses ! "


# DERNIERE ETAPE SI J'AI LE TEMPS => Peaufiner tout ça :
# Mettre les réponses à chaque question sous forme de methodes
# Faire une methode qui demande au user quelle question il veut poser et qui lance la methode correspondant au programme affichant la réponse

# checker le cours sur "REQUIRE" pour voir si c'est pas plus pratique de cette façon plutot qu'avec des méthodes pour faire une autre maniere de presenter l'exo ?
